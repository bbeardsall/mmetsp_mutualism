...
```{r}
join_taxid %>%
  select(asm_name, ftp_path) %>%
  mutate(genome_ftp_path = paste(
    paste(ftp_path, tail(str_split(ftp_path, '/')[[1]], 1), sep = "/"),
    suffix, sep = "")
  )
```
...
```{r}
string = 'ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCA/001/652/855/GCA_001652855.1_Pro_Min_D-127.v1.0'
tail(str_split(string, '/')[[1]], 1)
```

```{r}
ftp_paths <- join_taxid$ftp_path
```
```{r}
suffix <- "_genomic.fna.gz"
ftp_paths_suffix <- paste(ftp_paths, suffix, sep = "")
```
...
```{r}
https://ftp.ncbi.nlm.nih.gov/genomes/all/GCA/001/652/855/GCA_001652855.1_Pro_Min_D-127.v1.0/GCA_001652855.1_Pro_Min_D-127.v1.0_genomic.fna.gz
```
