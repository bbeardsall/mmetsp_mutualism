...
genbank_select <- genbank %>%
  select(taxid, genome_ftp_path, genome_filename) %>%
  ungroup() %>%
...
How many genus matches?
```{r}
sum(genbank_taxon_joined$genus %in% unique(mmetsp_unnested$genus))
```


```{r}
one_genus <- genbank_taxon_joined %>%
  filter(genus %in% unique(mmetsp_unnested$genus)) %>%
  group_by(genus) %>%
  top_n(1, wt = genus) %>%
  ungroup()
...
```
```{r}
sum(mmetsp_unnested$genus %in% one_genus$genus) / length(mmetsp_unnested$genus)

```

Family?
```{r}
sum(genbank_taxonomy_joined$family %in% unique(mmetsp_unnested$family))
```
